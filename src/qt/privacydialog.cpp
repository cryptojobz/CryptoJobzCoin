...
#include "zcjzcoincontroldialog.h"
#include "spork.h"
                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zCJZ ought to be enough for anybody." - Bill Gates, 2017
    ui->zCJZpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzCJZSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzCJZ->setEnabled(false);
        ui->pushButtonMintzCJZ->setToolTip(tr("zCJZ is currently disabled due to maintenance."));

        ui->pushButtonSpendzCJZ->setEnabled(false);
        ui->pushButtonSpendzCJZ->setToolTip(tr("zCJZ is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zCJZpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzCJZ_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zCJZ is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zCJZ...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zCJZ in ") +
...
void PrivacyDialog::on_pushButtonSpendzCJZ_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zCJZ is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zCJZ
        sendzCJZ();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zCJZ
    sendzCJZ();
}

void PrivacyDialog::on_pushButtonZCJZcoinControl_clicked()
{
    ZCJZcoinControlDialog* zCJZControl = new ZCJZcoinControlDialog(this);
    zCJZControl->setModel(walletModel);
    zCJZControl->exec();
}

void PrivacyDialog::setZCJZcoinControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzCJZSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzCJZ()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid CJZcoin Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zCJZpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zCJZ is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " CJZCOIN </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zCJZ</b>";
    ...
    // use mints from zCJZ selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZCJZcoinControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZCJZcoinControlDialog::GetSelectedMints();
    }
    ...
    // Clear zcjzcoin selector in case it was used
    ZCJZcoinControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zCJZ Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " CJZcoin, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zCJZ Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zCJZ </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zCJZ "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zCJZ "));
    ui->labelzCJZAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzCJZSyncStatus->setVisible(fShow);
}
