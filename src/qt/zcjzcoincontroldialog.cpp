...
#include "ui_zcjzcoincontroldialog.h"
...
std::list<std::string> ZCJZcoinControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZCJZcoinControlDialog::listMints;

ZCJZcoinControlDialog::ZCJZcoinControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZCJZcoinControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZCJZcoinControlDialog::~ZCJZcoinControlDialog()
{
    delete ui;
}

void ZCJZcoinControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZCJZcoinControlDialog::updateList()
{
...
void ZCJZcoinControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZCJZcoinControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZCJZcoin_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZCJZcoinControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZCJZcoinControlDialog::GetSelectedMints()
{
...
void ZCJZcoinControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
